...
def test_so2_paper_faithful_convergence_rate():
    """Virieux 1986 §II claim: 2nd-order accuracy in space.

    A 2-point centred staggered FD MUST achieve formal rate 2 on a
    smooth manufactured solution. Empirically rate is ~1.98 at
    N=16→32 refinement — comfortably above the 1.8 threshold."""
    rate, _ = _mms_convergence_rate(space_order=2)
    assert 1.8 <= rate <= 2.5, (
        f"SO=2 .dx convergence rate {rate:.3f} outside expected "
        f"2nd-order band [1.8, 2.5] — Virieux 1986 Eq 4 claim"
    )


def test_so4_paper_faithful_convergence_rate():
    """Levander 1988 §II claim: 4th-order accuracy in space."""
...
def test_so8_convergence_rate():
    """SO=8 is beyond formal scope of Virieux 1986 but included per
    repo-wide so=16 convention. Expected rate ≈ 8 at the coarsest
    refinement step before round-off saturates."""
