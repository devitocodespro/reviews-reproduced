"""Devito reproduction of Levander (1988) — fourth-order P-SV
velocity-stress staggered-grid finite-difference scheme.

Paper
-----
Levander, A.R. (1988). "Fourth-order finite-difference P-SV
seismograms." *Geophysics* 53(11), 1425-1436.
DOI 10.1190/1.1442422.

Scheme
------
Same 2D P-SV velocity-stress staggered-grid framework as
Virieux 1986 (see ``../doi_10.1190_1.1442147/``); Levander 1988
extends the spatial discretisation from 2nd-order to 4th-order
via Eq 4's 4-point centred-FD stencil with weights
``{1/24, -9/8, +9/8, -1/24} / Δx``. The continuous PDE system
is identical:
...
Yee staggering identical to Virieux 1986 Fig 1.
...
canonical Levander 1988 claim is at ``space_order=4``; higher
orders {8, 16} are documented as Taylor-truncation extensions
of the same scheme; the SO=2 case is included for cross-
validation against the companion Virieux 1986 reproduction at
``../doi_10.1190_1.1442147/``.
...
# Homogeneous isotropic material — same as the companion Virieux
# 1986 folder to allow direct numerical cross-validation between
# the two reproductions at shared space_order values.
VP_KMS = 3.0     # km/s
VS_KMS = 1.7     # km/s
RHO_GCC = 2.2    # g/cm³
...
def build_source_term(grid: Grid, tau, dt_value: float, nt: int):
    ...
    stress components. Domain-centre location."""
...
def run_reproduction(space_order: int, save_npz: bool = True,
    ...
    """Run the Levander 1988 reproduction at the given ``space_order``.
    ...
    op = Operator(eqs + src_terms, name=f'LevanderSO{space_order}')
...
def main():
    ...
    print(f'Levander 1988 reproduction — running '
          f'space_order ∈ {args.space_order}')
    for so in args.space_order:
        print(f'  space_order={so} '
              f'(canonical={so == 4}, dt={cfl_dt(build_grid(so), so):.4e} s)')
